...
const { createUploadsFolder, readFile, saveFile, appendFile } = require('./utils/fileManager');
...
io.on('connection', (socket) => {
	...
	socket.on('getChatMessages', async () => {
		try {
			...
			if (err.code === 'ENOENT') return io.emit('chatInfo', { info: 'No se encontraron mensajes' });
